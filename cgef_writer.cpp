...
void CgefWriter::storeCellBorder(short *borderPath, unsigned int cell_num) const {
    ...
    hid_t dataset_id =
        H5Dcreate(group_id_, "cellBorder", H5T_STD_I16LE, dataspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
void CgefWriter::storeCellBorder_cnt(vector<short> &borcnt) {
    ...
    hid_t dataspace_id = H5Screate_simple(1, dims, nullptr);
    hid_t dataset_id =
        H5Dcreate(group_id_, "cellBordercnt", H5T_STD_I16LE, dataspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
void CgefWriter::storeCellExp() {
    ...
    hid_t dataspace_id = H5Screate_simple(1, dims, nullptr);
    hid_t dataset_id = H5Dcreate(group_id_, "cellExp", filetype, dataspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
void CgefWriter::storeCell(unsigned int block_num, unsigned int *block_index, const unsigned int *block_size) {
    ...
    hid_t dataset_id = H5Dcreate(group_id_, "cell", filetype, dataspace_id, H5P_DEFAULT, dpid, H5P_DEFAULT);
    ...
    if (cell_num_ % 2 != 0) {
        ...
        cell_attr_.median_gene_count =
            float(cell_list_[index[mi]].gene_count + cell_list_[index[mi - 1]].gene_count) / 2;
    }

    iota(index, index + cell_num_, 0);
    sort(index, index + cell_num_, [this](int a, int b) { return cell_list_[a].exp_count < cell_list_[b].exp_count; });
    if (cell_num_ % 2 != 0) {
    ...
    iota(index, index + cell_num_, 0);
    sort(index, index + cell_num_, [this](int a, int b) { return cell_list_[a].dnb_count < cell_list_[b].dnb_count; });
    if (cell_num_ % 2 != 0) {
...
void CgefWriter::storeBlkidx(unsigned int block_num, unsigned int *block_index, const unsigned int *block_size) {
    ...
    hid_t dataset_id = H5Dcreate(group_id_, "blockIndex", H5T_STD_U32LE, ds, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    ...
    hid_t sdataset_id = H5Dcreate(group_id_, "blockSize", H5T_STD_U32LE, sds, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
void CgefWriter::storeCellLabel(vector<unsigned int> &vecdata) {
    ...
    hid_t ds = H5Screate_simple(1, dims, nullptr);
    hid_t dataset_id = H5Dcreate(group_id_, "label", H5T_STD_U32LE, ds, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
void CgefWriter::addDnbExp(vector<cv::Point> &dnb_coordinates,
                           ...
                           const DnbExpression *dnb_expression, const cv::Point &center_point, unsigned short area) {
        ...
        if (iter != bin_gene_exp_map.end()) {
            ...
            for (unsigned int i = gene_info.first; i < end; i++) {
                ...
                if (iter_gene != gene_count_in_cell.end()) {
                    ...
                    gene_count_in_cell.insert(map<unsigned int, unsigned short>::value_type(dnb_expression[i].gene_id,
                                                                                            dnb_expression[i].count));
                    gene_count++;
                }
            }
        }
...
void CgefWriter::storeAttr(CellBinAttr &cell_bin_attr) {
    ...
    H5Awrite(bin_attr, str32_type_, "CellBin");
    H5Sclose(bin_did);
    H5Aclose(bin_attr);
...
void CgefWriter::storeGeneAndGeneExp(unsigned int min_exp_count, unsigned int max_exp_count,
                                     unsigned int min_cell_count, unsigned int max_cell_count, GeneData *gene_data_list,
                                     vector<GeneExpData> &gene_exp_list) {
    ...
    dataset_id = H5Dcreate(group_id_, "geneExp", filetype, dataspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
void CgefWriter::storeCellTypeList() {
    ...
    hid_t dataspace_id = H5Screate_simple(1, dims, nullptr);
    hid_t dataset_id =
        H5Dcreate(group_id_, "cellTypeList", str32_type_, dataspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
int CgefWriter::write(BgefReader &common_bin_gef, Mask &mask) {
    ...
    for (unsigned int i = 0; i < mask.getCellNum(); i++) {
        ...
        addDnbExp(non_zero_coordinates_offset, bin_gene_exp_map, dnb_exp_info, p.getCenter(), p.getAreaUshort());
    ...
    CellBinAttr cell_bin_attr = {/*.version = */ 1,
...
unsigned short CgefWriter::getRandomCellTypeNum() const { return random_cell_type_num_; }

void CgefWriter::setRandomCellTypeNum(unsigned short random_cell_type_num) {
    random_cell_type_num_ = random_cell_type_num;
}

bool CgefWriter::isVerbose() const { return verbose_; }

void CgefWriter::setVerbose(bool verbose) { verbose_ = verbose; }
...
int CgefWriter::addLevel(int allocat, int cnum, float ratio, int *cansize, int *blknum) {
    if (cansize[0] <= (m_canvas[0] + m_offsetX) && cansize[2] >= (m_canvas[2] + m_offsetX) &&
        cansize[1] <= (m_canvas[1] + m_offsetY) && cansize[3] >= (m_canvas[3] + m_offsetY)) {
...
void CgefWriter::writeCelldata(int lev, int *blknum, vector<block> &blk, vector<int> &vecid, vector<int> &vec_blk_idx) {
    ...
    hid_t blk_dset_id =
        H5Dcreate(level_gid, "blk", m_blk_memtype, blk_dspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    ...
    hid_t id_dset_id =
        H5Dcreate(level_gid, "cellid", H5T_NATIVE_UINT32, id_dspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    ...
    hid_t bid_dset_id =
        H5Dcreate(level_gid, "noempty", H5T_NATIVE_UINT32, bid_dspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
void CgefWriter::storeGeneExon(uint32_t minExon, uint32_t maxExon, uint32_t *geneExonPtr, uint16_t maxExpExon,
    ...
    hid_t exonExp_did =
        H5Dcreate(group_id_, "geneExpExon", H5T_STD_U16LE, exonExp_sid, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
...
void CgefWriter::storeCellExon(uint16_t minExon, uint16_t maxExon, vector<uint16_t> vec_cellexon, uint16_t maxExpExon,
    ...
    hid_t exonExp_sid = H5Screate_simple(1, dims, nullptr);
    hid_t exonExp_did =
        H5Dcreate(group_id_, "cellExpExon", H5T_STD_U16LE, exonExp_sid, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
