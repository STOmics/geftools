...
{
public:
    getleveldnbtask(bool blevel, bool btop, int bin, uint32_t start, 
    uint32_t len, uint32_t cols, uint32_t offset_x, uint32_t offset_y, 
    uint32_t maxmid, level_fractal &lf, dnbbuf &dbuf, 
    BinStatUS *pdataus = nullptr, BinStat *pdata = nullptr):
    blevel_(blevel), btop_(btop), bin_(bin), start_(start), len_(len),
    cols_(cols), offset_x_(offset_x), offset_y_(offset_y), maxmid_(maxmid),
    lf_(lf), dbuf_(dbuf), pdataus_(pdataus), pdata_(pdata)
    {
    ...
    bool leveltop(uint32_t x, uint32_t y)
    {
        ...
        bool xret = mx == lf_.start_fractal_coor |
                    mx == lf_.mid_fractal_coor |
                    mx == lf_.end_fractal_coor;
        if(xret)
        {
            uint32_t my = y % lf_.fractal_num;
            bool yret = my == lf_.start_fractal_coor |
                        my == lf_.mid_fractal_coor |
                        my == lf_.end_fractal_coor;
            if(yret)
            {
                return true;
            }
            ...
            if(btop_)
            {
                for(uint32_t i=0;i<len_;i++)
                {
                    if(pdataus_[i].gene_count)
                    {
                        ...
                        if(ret)
                        {
                            m_vecdnb.emplace_back(x,y,pdataus_[i].gene_count, pdataus_[i].mid_count, pdataus_[i].mid_count*1.0/maxmid_);
                        }
                    }
                }
            }
            ...
            if(btop_)
            {
                for(uint32_t i=0;i<len_;i++)
                {
                    if(pdata_[i].gene_count)
                    {
                        ...
                        if(ret)
                        {
                            m_vecdnb.emplace_back(x,y,pdata_[i].gene_count, pdata_[i].mid_count, pdata_[i].mid_count*1.0/maxmid_);
                        }
                    }
                }
            }
