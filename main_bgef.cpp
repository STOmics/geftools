...
int bgef(int argc, char *argv[]) {
    ...
    options.set_width(120).add_options()("i,input-file",
                                         "input gene expression matrix file(.gem/.gem.gz) or bin1 bGEF file [request]",
        ...
        cxxopts::value<std::string>()->default_value("1,10,20,50,100,200,500"),
        "STR")("r,region",
               "Restrict to a rectangular region. The region is represented by the comma-separated list"
...
int generateBgef(const string &input_file, const string &bgef_file, const string &stromics, int n_thread,
                 vector<unsigned int> bin_sizes, vector<int> region, bool verbose, bool bstat) {
...
void gem2gef(BgefOptions *opts) {
    ...
    for (unsigned int bin : opts->bin_sizes_) {
        ...
        if (bin == 1) {
            ...
            for (auto &p : opts->map_gene_exp_) totalSize += p.second.size();
...
unsigned int Boxplot(vector<unsigned int> &vecmid) {
    sort(vecmid.begin(), vecmid.end(), [](const unsigned int a, const unsigned int b) { return a < b; });
    int sz = vecmid.size();
...
void MergeProteinAndRnaMatrices(const string &protein_raw_gef, const string &rna_raw_gef,
    ...
    if (offset_x == 0) {
        ...
        matrix_max_x = protein_exp_attr.max_x >= rna_exp_attr.max_x ? protein_exp_attr.max_x : rna_exp_attr.max_x;
    } else if (offset_x > 0) {
        for (unsigned long long i = 0; i < protein_exp_count; i++) {
            protein_exp_data[i].x += offset_x;
        }
        matrix_min_x = rna_exp_attr.min_x;
        matrix_max_x = (protein_exp_attr.max_x + offset_x) >= rna_exp_attr.max_x ? (protein_exp_attr.max_x + offset_x)
                                                                                 : rna_exp_attr.max_x;
    } else {
        ...
        matrix_min_x = protein_exp_attr.min_x;
        matrix_max_x = (rna_exp_attr.max_x - offset_x) >= protein_exp_attr.max_x ? (rna_exp_attr.max_x - offset_x)
                                                                                 : protein_exp_attr.max_x;
    }

    int offset_y = protein_exp_attr.min_y - rna_exp_attr.min_y;
    if (offset_y == 0) {
        matrix_min_y = protein_exp_attr.min_y;
        matrix_max_y = protein_exp_attr.max_y >= rna_exp_attr.max_y ? protein_exp_attr.max_y : rna_exp_attr.max_y;
    } else if (offset_y > 0) {
        for (unsigned long long i = 0; i < protein_exp_count; i++) {
            protein_exp_data[i].y += offset_y;
        }
        matrix_min_y = rna_exp_attr.min_y;
        matrix_max_y = (protein_exp_attr.max_y + offset_y) >= rna_exp_attr.max_y ? (protein_exp_attr.max_y + offset_y)
                                                                                 : rna_exp_attr.max_y;
    } else {
        for (unsigned long long i = 0; i < rna_exp_count; i++) {
        ...
        matrix_min_y = protein_exp_attr.min_y;
        matrix_max_y = (rna_exp_attr.max_y - offset_y) >= protein_exp_attr.max_y ? (rna_exp_attr.max_y - offset_y)
                                                                                 : protein_exp_attr.max_y;
    }
...
void Gem2Image(const string &gem_path, const string &tif_path) {
    ...
    ThreadPool thpool(8);
    for (int i = 0; i < 8; i++) {
        auto *rtask = new GetCoordinateTask(gem_file, col, image_range, coordinate_info);
        thpool.addTask(rtask);
    }
    ...
    int row = image_range[3] - offset_y + 1;
    int column = image_range[1] - offset_x + 1;
    cv::Mat tiff_image = cv::Mat::zeros(row, column, CV_8UC1);
    for (auto iter : coordinate_info) {
        tiff_image.at<uchar>((iter.y - image_range[2]), (iter.x - image_range[0])) = 255;
    }
