...
class GEFTOOLS_API bgefCreater {
  public:
    ...
    void getStereoData(const string &strin, int maskbin, const string &strmask, vector<string> &vec_gene,
...
class GEFTOOLS_API gemmaskTask : public ITask {
  public:
    gemmaskTask(uint32_t gid, bgefCreater *ptr) : m_gid(gid), m_ptr(ptr) {}
    ~gemmaskTask() {}
    void doTask() {
        gdata *pdata = new gdata();
        pdata->geneid = m_gid;
