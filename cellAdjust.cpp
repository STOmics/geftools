...
void cellAdjust::createRegionGef(const string &out) {
    ...
    for (unsigned int bin : m_bgefopts->bin_sizes_) {
        ...
        while (true) {
            ...
            if (bin == 1) {
                m_bgefopts->expressions_.insert(m_bgefopts->expressions_.end(), pgeneinfo->vecptr->begin(),
            ...
            m_bgefopts->genes_.emplace_back(pgeneinfo->geneid, offset,
...
void cellAdjust::getMultiLabelInfoFromCgef(const string &strcgef, vector<vector<int>> &vecpos,
    ...
    attr = H5Aopen(file_id, "resolution", H5P_DEFAULT);
    H5Aread(attr, H5T_NATIVE_UINT, &m_resolution);
    std::cout << "offsetx:" << m_offsetX << "offsety:" << m_offsetY << "resolution:" << m_resolution << '\n';
    H5Tclose(memtype);
    H5Sclose(cell_sid);
    H5Dclose(cell_did);
    H5Aclose(attr);
