...
int cgef(int argc, char *argv[]) {
    ...
    options.set_width(120).add_options()("i,input-file", "input GEF file [request]", cxxopts::value<std::string>(),
                                         "FILE")("m,mask-file", "input mask file [request]",
    ...
    if (patch == 1) {
        ...
        cgef3d c3d;
        c3d.writeCgef(cgefParam::GetInstance()->m_inputstr, cgefParam::GetInstance()->m_rawgemstr,
                      cgefParam::GetInstance()->m_maskstr, cgefParam::GetInstance()->m_outputstr);
    } else if (patch == 0) {
