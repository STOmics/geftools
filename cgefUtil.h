...
class cgef_cell {
  public:
    ...
    cgef_cell(int label, char *ptr, int len) : m_celllabel(label) {
...
class bgef_gene {
  public:
    bgef_gene() {};
    ~bgef_gene() {};
    void add(uint32_t x, uint32_t y, uint32_t midcnt) { m_vecExp.emplace_back(x, y, midcnt); }
    void merge(bgef_gene &other) { m_vecExp.insert(m_vecExp.end(), other.m_vecExp.begin(), other.m_vecExp.end()); }
...
struct cellExp_Exon {
    cellExp_Exon(uint32_t gid, uint16_t mid, uint16_t exon) : geneid(gid), midcnt(mid), exoncnt(exon) {};
    uint32_t geneid;
    uint16_t midcnt;
    uint16_t exoncnt;
};
...
struct cexp {
    cexp(uint32_t cid, uint16_t mid, uint16_t exon) : cellid(cid), midcnt(mid), exoncnt(exon) {}
    uint16_t midcnt;
    uint16_t exoncnt;
    uint32_t cellid;
};
