...
class GEFTOOLS_API BgefReader {
  ...
  public:
     ...
     */
    void readWholeExpMatrix(unsigned int offset_x, unsigned int offset_y, unsigned int rows, unsigned int cols,
     ...
     * @param matrix  When the value is greater than 255, it will be set to 255.
     */
    void readWholeExpMatrix(string &key, unsigned char *matrix);
    ...
    void getSparseMatrixIndicesOfExp(vector<unsigned long long> &uniq_cells, unsigned int *cell_ind,
                                     unsigned int *count);
    ...
    bool isExonExist() { return m_exonPtr != nullptr; }
